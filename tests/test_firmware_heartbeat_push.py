...
def test_transport_uses_short_timeouts_and_a_watchdog_fuse():
    body = _fn("push_post_json")
    assert "http.setConnectTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);" in body
    assert "http.setTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);" in body
    assert "client.setTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);" in body
    assert "boot_wdt_arm(HEAR_PUSH_WDT_MS);" in body
    assert "http.POST((uint8_t *)body, body_len);" in body
    assert "boot_wdt_disarm();" in body
