...
def _wav(path, samples, fs=16000):
...
def _tone(n, amp, fs=16000, f=440.0):
...
def _pool(tmp_path, rows):
    ...
    for r in rows:
        ...
        _wav(str(tmp_path / "corpus" / r["path"]), r.pop("_samples"), r.pop("_fs", 16000))
        idx.append(r)
...
def _row(node, day, name, samples, **kw):
    r = {"outcome": "stored", "node": node, "clip_key": name,
         ...
         "anchored": True, "trigger": "175", "wav_header_fs_hz": 16000, "fs_hz": 16000.0,
...
def test_sample_is_balanced_by_node_not_by_day_count(tmp_path):
    ...
    for d in ("2026-09-08", "2026-09-09", "2026-09-10"):
        for i in range(20):
            rows.append(_row("mach", d, "m-%s-%d" % (d, i), _tone(1600, 200)))
    for i in range(20):
        rows.append(_row("rankine", "2026-09-10", "r-%d" % i, _tone(1600, 200)))
...
def test_required_node_absence_is_reported_not_silent(tmp_path):
    ...
    pool = _pool(tmp_path / "p", [_row("nyquist", "2026-09-10", "a", _tone(1600, 200))])
    out = str(tmp_path / "out")
...
def test_missing_audio_is_skipped_and_counted(tmp_path):
    ...
    rows = [_row("mach", "2026-09-10", "a", _tone(1600, 200)),
            _row("mach", "2026-09-10", "b", _tone(1600, 200))]
    pool = _pool(tmp_path / "p", rows)
...
def test_sheet_has_one_blank_row_per_staged_clip(tmp_path):
    rows = [_row("mach", "2026-09-10", "c%d" % i, _tone(1600, 200)) for i in range(5)]
    pool = _pool(tmp_path / "p", rows)
    out = str(tmp_path / "out")
