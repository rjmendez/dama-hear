...
class TestAdmit:
    ...
    def test_missing_sample_rate_and_layout_remain_absent_in_the_audit(self, tmp_path):
        """TDoA uses timestamps, not the sketch band axis, so an old row may still participate.
        Its missing measurement metadata must remain visible rather than acquiring defaults."""
        rec = P._record_from_node_row(node_row("nyquist", T0, seed=12))
        rec["fs_hz"] = None
        rec["layout"] = None
        r, t = self._reasons(tmp_path, [], extra=[rec])
        assert r.get(HT.D_ADMITTED) == 1
        ledger = [json.loads(line) for p in (pathlib.Path(t["out"]) / "arrivals").rglob("*.jsonl")
                  for line in p.read_text().splitlines()]
        assert ledger[0]["fs_hz"] is None
        assert ledger[0]["layout"] is None
...
class TestResumeIsIdempotent:
    ...
    def test_the_same_members_under_two_source_profiles_have_distinct_event_keys(self, tmp_path):
        """A blast and an impact are different asserted source profiles even when they happen
        to select the same point-source solver and the same member rows."""
        sv = SV.from_dict(survey_dict())
        rows = planted(sv, (40.0, 30.0, 0.0), T0)
        build_pool(tmp_path / "pool", rows)
        out = str(tmp_path / "out")
        ...
        blast = HT.run(str(tmp_path / "pool"), survey, pol(source_class="blast"),
                       out=out, now=T0 + 3600.0)
        impact = HT.run(str(tmp_path / "pool"), survey, pol(source_class="impact"),
                        out=out, now=T0 + 7200.0)

        def keys(run):
            p = pathlib.Path(out) / "runs" / run["run_id"] / "events.jsonl"
            return [json.loads(line)["event_key"] for line in p.read_text().splitlines()]

        assert blast["events_new"] == impact["events_new"] == 1
        assert keys(blast) != keys(impact)
