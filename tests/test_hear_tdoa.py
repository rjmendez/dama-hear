...
class TestNoTerminalVerdictGoesOutWithoutAReason:
    ...
    ⚠️THIS DOES NOT FIX THAT DIVERGENCE, it only makes the verdict legible. An event whose
    members the driver never attempts is a separate defect; this test is what stops that defect
    being invisible while it is open.
    """
    ...
    def test_every_lost_to_gate_row_names_why(self, tmp_path):
        ...
        lost = [a for a in attempts if a["verdict"] == HT.V_LOST_TO_GATE]
        assert lost, "this fixture must produce one, or it is not testing the reason lookup"
        for a in lost:
            assert a["associate_reason"], (
                "verdict %r with associate_reason %r: the tool's charter is that every "
                "refusal names its reason" % (a["verdict"], a["associate_reason"]))
        assert any(a["associate_reason"].startswith("member_of_event") for a in lost)
        # and the candidates.jsonl row carries the same reason, not a second opinion
        cands = {c["candidate_id"]: c for c in
                 (json.loads(l) for l in (run / "candidates.jsonl").read_text().splitlines())}
        for a in lost:
            assert cands[a["candidate_id"]]["associate_reason"] == a["associate_reason"]
