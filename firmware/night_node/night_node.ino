...
// of it -- the gate, the sketch, the scene bank, the PPS timebase, dets -- means exactly that. The
// microphone now runs FASTER than that, and the two must never be confused: the mel banks in
// mel16.h and mel_scene.h are GENERATED TABLES with 16000.0f baked into their band edges, so a
// filterbank applied to audio at any other rate silently attributes every band to the wrong
// frequency while the CSV keeps declaring f_lo=62.5, f_hi=7812.5. Nothing would raise. The
// static_asserts below are what make that unrepresentable rather than merely documented.
//
...
// input -- which is why a clip cut at J*DECIM alone would start 4.00 ms late.
...
// ⚠️THE GUARD THAT DID NOT EXIST. FS_NOMINAL and the mel banks' baked-in rate were free to drift
// apart: raising FS_NOMINAL compiled clean, ran the I2S faster, and kept applying a filterbank
// computed for 16 kHz -- every band silently covering a different frequency than the row it is
// written into declares. This is the whole reason the decimator exists rather than a bare rate
// change, so it is asserted, not commented.
static_assert((int)MEL16_FS == FS_NOMINAL, "sketch mel bank was generated for a different rate");
...
static void audio_pump() {
  { size_t got = i2s.readBytes((char *)blk, sizeof blk);
    ...
    if (!dc_ready && n > 0) {
      ...
      // ---- pass 1, AT FS_ACQ: DC-block only. This is the clip stream.
      for (int i = 0; i < n; i++) {
      ...
      for (int i = 0; i < nd; i++) {
        ...
        uint32_t idx = (det_n++) % MAXDET;
        {
          ...
          // the FIR group delay: the sound reached the microphone 128 acquisition samples -- exactly
          // 4.00 ms -- before it reached this output sample. Omitting it would stamp every detection
          // 4 ms late and throw away far more than the 21 ns the GPS is handing us.
          double   fsu     = fs_timebase();
