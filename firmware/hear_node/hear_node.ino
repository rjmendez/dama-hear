...
#define HEAR_PUSH_BASE_URL          "http://mrpink"
#define HEAR_PUSH_HEARTBEAT_PATH    "/api/hear/heartbeat"
#define HEAR_PUSH_EVENT_PATH        "/api/hear/event"
#define HEAR_PUSH_HTTP_TIMEOUT_MS   75u
#define HEAR_PUSH_WDT_MS            1000u
...
static bool push_post_json(const char *path, const char *body, size_t body_len, int *code_out) {
  ...
  char url[96];
  int n = snprintf(url, sizeof url, "%s%s", HEAR_PUSH_BASE_URL, path);
  ...
  client.setTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);
  HTTPClient http;
  http.setReuse(false);
  http.setConnectTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);
  http.setTimeout(HEAR_PUSH_HTTP_TIMEOUT_MS);
