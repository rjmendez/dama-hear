...
Measured on the 2026-09-07 11.33 h capture. That gate's floor is 800 counts against a median
ambient of 29.4 — 28.7 dB over the background, and 19.5 dB over ambient p95 (84.8). A chorus a
plausible 10 dB over ambient sits ~19 dB below it and cannot fire it at any hour.

Its envelope is also broadband, and the night's own sketches price that. **Recipe, so the two
numbers below can be checked rather than believed:** `dets.csv` holds 59 rows; 11 set flags bit 1
(*insufficient context* — the sketch is of a ring that had not filled) and are excluded, leaving
48. Decode each `frame_hex` with `hear.sketch.unpack`, take the `db` field (20 bands × 8 frames)
and reduce each band to its **maximum over the 8 frames**. Then:

- **46 of the 48 peak in mel band 0.** Band 0's only nonzero rfft bins at fs = 16 kHz, nfft = 256
  are **312.5–500 Hz** (its mel triangle nominally spans 300–527 Hz). That is as precisely as the
  peak can be located: a peak in band 0 is equally consistent with a source *inside* 312–500 Hz
  and with one below 300 Hz that the filterbank cannot see. The conclusion that survives is the
  weaker one — **the energy sat at the bottom edge of what the node represents**.
- **Band 0 runs a median 20.4 dB above the 4–8 kHz bands**, where "the 4–8 kHz bands" means the
  mean of bands 16–19, the four whose triangular support lies entirely above 4 kHz (4425–7840 Hz).

The reduction is part of the measurement: per-band *mean* over frames instead of max gives 45 of
48 and 16.5 dB, and comparing against the *loudest* of bands 16–19 rather than their mean gives
17.2 dB. An earlier revision of this file claimed "all 45 … a median 22.3 dB"; 22.3 reproduces
under none of those recipes and is gone, and "all 45" was an inference stated as a count.
...
- A `block_start` **ahead** of where the last block ended is a hole in the stream. The gate
  ...
  **A gap never passes silently**, because a dropped block used to shift every subsequent `index`,
  ...
  totalling `drop_samples=42749` — about **2.7 s** of audio, not 18 s — over
  11.33 h.
...
- **`n_unstructured` counts stretches; `n_unstructured_segments` counts discards.** A shapeless
  ...
  60 s wind gust would score 6 at the shipped 10 s limit and 3 at 20 s. Only the stretch count is
...
- **`floor_rise_db` replaced a `floor_absorbed` flag that could never fire.** The flag claimed to
  ...
  force-closed anyway — clearing `close_hyst_db` would take 91.7 dB of in-band SNR. It was
  `False` on every event at 12, 30, 60 and 90 dB. What absorption really looks like is a
...
inline in `detect.py`. The night capture contains no biological example to fit them to: grouping
its 48 usable detections at a 1 s gap gives 32 low-frequency impulsive events, 46 of the 48 peak
in mel band 0 and the other two in bands 9 and 12 (1807–2375 Hz and 2706–3478 Hz) — nothing
anywhere near the insect band. These are provisional numbers, to be refitted the first night a
person labels something real.
