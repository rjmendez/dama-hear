...
# A single bounded replay pass cannot clear a backlog bigger than its limit, so replay drains in
# batches and a background pass keeps resuming: an outage longer than the limit must not need a
# process restart per 256 rows.
DURABLE_REPLAY_MAX_BATCHES = int(os.environ.get("HEAR_DURABLE_REPLAY_MAX_BATCHES", "1024"))
DURABLE_REPLAY_INTERVAL_S = float(os.environ.get("HEAR_DURABLE_REPLAY_INTERVAL_S", "60"))
...
EVENT_PUBLISH_GUARD_PREFIX = os.environ.get("HEAR_EVENT_PUBLISH_GUARD_PREFIX", "dama:hear:published:")
EVENT_PUBLISH_GUARD_TTL_S = int(os.environ.get("HEAR_EVENT_PUBLISH_GUARD_TTL_S", "86400"))
...
class DurableRecordStore:
    ...
    def is_cached(self, record_uid: str) -> bool:
        return False
...
class RedisHeartbeatCache:
    # ⚠️XADD IS THE ONE NON-IDEMPOTENT CACHE WRITE. Every other key write here is a plain
    # SET/SETEX/SADD that can be replayed as often as the outbox needs, but an event XADD appends
    # a new stream entry every time it runs. A crash between the publish and the durable success
    # marker -- or two concurrent requests carrying the same record_uid -- would otherwise put the
    # same hear/event on dama:hear:events twice behind a single durable row. SET NX on a
    # per-record_uid guard key makes the append happen at most once, and the guard is taken and
    # used inside one Lua script so the reservation and the append cannot be split by a crash.
    _PUBLISH_ONCE_LUA = """
    if redis.call('SET', KEYS[1], ARGV[1], 'NX', 'EX', tonumber(ARGV[2])) then
      redis.call('XADD', KEYS[2], 'MAXLEN', '~', tonumber(ARGV[3]), '*',
                 'device_id', ARGV[4], 'payload', ARGV[5])
      return 1
    end
    return 0
    """
    ...
    def __init__(self, client: Any, heartbeat_ttl_s: int = HEARTBEAT_TTL_S,
                 ...
                 publish_guard_prefix: str = EVENT_PUBLISH_GUARD_PREFIX,
                 publish_guard_ttl_s: int = EVENT_PUBLISH_GUARD_TTL_S):
        self.client = client
        self.heartbeat_ttl_s = heartbeat_ttl_s
        self.event_stream_key = event_stream_key
        self.event_stream_maxlen = event_stream_maxlen
        self.publish_guard_prefix = publish_guard_prefix
        self.publish_guard_ttl_s = publish_guard_ttl_s
        self._eval_supported = True

    def write(self, record: Dict[str, Any], body_json: str,
              record_uid: Optional[str] = None) -> None:
        ...
        if telemetry_path == "hear/heartbeat":
            ...
            pipe.execute()
            self.publish_event_once(node_id, body_json, record_uid)
        else:  # pragma: no cover - validators own this in practice.
            raise ValueError(f"unsupported telemetry_path {telemetry_path!r}")

    def publish_event_once(self, node_id: str, body_json: str,
                           record_uid: Optional[str]) -> None:
        fields = {"device_id": node_id, "payload": body_json}
        if not record_uid:  # pragma: no cover - callers always pass one.
            pipe = self.client.pipeline(transaction=False)
            pipe.xadd(self.event_stream_key, fields,
                      maxlen=self.event_stream_maxlen, approximate=True)
            pipe.execute()
            return
        guard_key = f"{self.publish_guard_prefix}{record_uid}"
        if self._eval_publish_once(guard_key, node_id, body_json) is not None:
            return
        self._guarded_publish_once(guard_key, fields)

    def _eval_publish_once(self, guard_key: str, node_id: str,
                           body_json: str) -> Optional[int]:
        """Atomic guard+append; ``None`` means this client cannot run scripts at all."""
        if not self._eval_supported:
            return None
        try:
            return int(self.client.eval(
                self._PUBLISH_ONCE_LUA, 2, guard_key, self.event_stream_key,
                utc_now(), self.publish_guard_ttl_s, self.event_stream_maxlen,
                node_id, body_json,
            ) or 0)
        except (AttributeError, NotImplementedError, redis.exceptions.ResponseError) as exc:
            # Scripting off (or a stub client): fall back to the two-step guard below. Connection
            # and timeout errors deliberately propagate so a cache outage stays retryable.
            self._eval_supported = False
            logger.warning("redis eval unavailable (%s); using non-atomic publish guard",
                           _error_text(exc))
            return None

    def _guarded_publish_once(self, guard_key: str, fields: Dict[str, str]) -> None:
        if not self.client.set(guard_key, utc_now(), nx=True, ex=self.publish_guard_ttl_s):
            return
        try:
            pipe = self.client.pipeline(transaction=False)
            pipe.xadd(self.event_stream_key, fields,
            ...
            # Release the reservation so the outbox replay can publish this record later.
            try:
                self.client.delete(guard_key)
            except Exception as release_exc:  # pragma: no cover - best effort only.
                logger.warning("could not release publish guard %s: %s",
                               guard_key, _error_text(release_exc))
            raise
...
class HeartbeatReceiverStore:
    def __init__(self, client: Any, heartbeat_ttl_s: int = HEARTBEAT_TTL_S,
        ...
        self._uid_locks: Dict[str, list] = {}

    @contextlib.contextmanager
    def _record_lock(self, record_uid: str):
        """Serialise the whole persist -> publish -> mark-success path for one record_uid.

        The sqlite statements were already serialised, but two concurrent requests carrying the
        same record_uid could still interleave between the "already cached?" check and the cache
        publish and publish the same record twice.
        """
        with self._uid_locks_guard:
            entry = self._uid_locks.get(record_uid)
            if entry is None:
                entry = [threading.Lock(), 0]
                self._uid_locks[record_uid] = entry
            entry[1] += 1
        try:
            with entry[0]:
                yield
            ...
            with self._uid_locks_guard:
                entry[1] -= 1
                if entry[1] <= 0:
                    self._uid_locks.pop(record_uid, None)
    ...
    def _write(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._record_lock(record_uid):
            record = dict(payload)
            record["received_at"] = utc_now()
            record["receiver_schema_version"] = RECEIVER_SCHEMA_VERSION
            body_json = encode_json(record)
            stored = self.durable_store.persist(record_uid, record, body_json)
            if stored.cached:
                return stored.record
            self._publish(stored)
            return stored.record

    def _publish(self, stored: DurableEntry) -> None:
        """Cache-publish one already-durable record, then append its success marker."""
        try:
            self.cache.write(stored.record, stored.body_json, record_uid=stored.record_uid)
    ...
    def replay_pending(self, limit: int = DURABLE_REPLAY_LIMIT) -> Dict[str, Any]:
        ...
        for entry in pending:
            with self._record_lock(entry.record_uid):
                if self.durable_store.is_cached(entry.record_uid):
                    # A concurrent request published it between the pending scan and here.
                    summary["synced"] += 1
                    continue
                try:
                    self._publish(entry)
    ...
    def drain_pending(self, limit: int = DURABLE_REPLAY_LIMIT,
                      max_batches: int = DURABLE_REPLAY_MAX_BATCHES) -> Dict[str, Any]:
        """Replay in bounded batches until nothing is pending (or the cache stops accepting).

        One ``replay_pending(limit=N)`` pass can only clear N rows, so an outage that left more
        than N pending records used to strand the remainder until the next process restart.
        """
        summary = {
            "backend": self.durable_store.backend,
            "attempted": 0,
        ...
        while max_batches <= 0 or summary["batches"] < max_batches:
            batch = self.replay_pending(limit=limit)
            summary["batches"] += 1
            summary["attempted"] += batch["attempted"]
            summary["synced"] += batch["synced"]
            summary["failed"] += batch["failed"]
            summary["remaining_pending"] = batch["remaining_pending"]
            if batch["attempted"] == 0 or batch["synced"] == 0:
                # Nothing pending, or the cache is refusing every write: stop and let the
                # background pass retry rather than spin against an outage.
                break
            if batch["remaining_pending"] == 0:
                break
        summary["drained"] = summary["remaining_pending"] == 0
        return summary
...
class ReplayWorker(threading.Thread):
    """Recurring bounded drain pass so a backlog never waits for the next process restart."""

    def __init__(self, store: "HeartbeatReceiverStore", interval_s: float = DURABLE_REPLAY_INTERVAL_S,
                 limit: int = DURABLE_REPLAY_LIMIT,
                 max_batches: int = DURABLE_REPLAY_MAX_BATCHES,
                 on_summary: Optional[Callable[[Dict[str, Any]], None]] = None):
        super().__init__(name="hear-durable-replay", daemon=True)
        self.store = store
    ...
    def run_once(self) -> Dict[str, Any]:
        summary = self.store.drain_pending(limit=self.limit, max_batches=self.max_batches)
        if self.on_summary is not None and (summary["attempted"] or summary["failed"]):
    ...
    def run(self) -> None:
        while not self._stop_event.wait(self.interval_s):
            try:
                self.run_once()
            except Exception as exc:  # pragma: no cover - a replay pass must never kill the loop.
                logger.warning("durable replay pass failed: %s", _error_text(exc))
...
def start_replay_worker(store: "HeartbeatReceiverStore",
                        interval_s: float = DURABLE_REPLAY_INTERVAL_S,
                        limit: int = DURABLE_REPLAY_LIMIT,
                        max_batches: int = DURABLE_REPLAY_MAX_BATCHES,
                        on_summary: Optional[Callable[[Dict[str, Any]], None]] = None
                        ) -> Optional[ReplayWorker]:
    if not store.durable_store.enabled or interval_s <= 0 or limit <= 0:
        return None
    worker = ReplayWorker(store, interval_s=interval_s, limit=limit,
                          max_batches=max_batches, on_summary=on_summary)
    worker.start()
    return worker
...
def add_durable_store_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--durable-replay-max-batches", type=int, default=DURABLE_REPLAY_MAX_BATCHES)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    start_replay_worker(
        ...
        max_batches=args.durable_replay_max_batches,
        on_summary=lambda s: print(
            "[hear-heartbeat] replay pass attempted=%d synced=%d failed=%d remaining=%d"
