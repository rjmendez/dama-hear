...
def main(argv=None) -> int:
    ...
    report = P.scan_pool(target, engine=args.vad, threshold=args.threshold,
                         min_speech_ms=args.min_speech_ms, dry_run=args.dry_run,
                         audit_log=args.audit_log, vad=vad, on_outcome=emit)

    summary = report.as_record()
    if args.json:
        print(json.dumps({"summary": summary}, sort_keys=True, separators=(",", ":")))
    else:
        print("scanned %d  purged %d  would_purge %d  kept %d  errors %d  vad %s%s"
              % (report.scanned, report.purged, report.would_purge, report.kept, report.errors,
                 report.vad_engine,
