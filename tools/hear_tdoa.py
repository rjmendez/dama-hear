...
def scan_coincidences(dets: Sequence[Dict[str, Any]], window_s: float,
    ...
    ⚠️NO LONGER "SAME SCAN, MINUS ONE GATE" -- THAT CLAIM WENT FALSE UNDER #42 AND WAS NEVER
    UPDATED. `associate()` now has two NON-terminal refusals that return a candidate to ITS OWN
    pool for a later group to pick up as a fresh seed (hear/backend/associate.py: the
    ...
    unconditionally, so the two functions can and do seed different groups from the same
    detections -- measured on the live pool 2026-09-11: associate() seeds one event at
    node3@1789063974.287743, this scan seeds the corresponding group eight ms later at
    node1@.299200. Anything that reads this scan's groups alongside associate()'s own events by
    matching on the seed (run()'s `ev_by_seed` lookup) can therefore miss an admissible group
    silently. See
    tests/test_hear_tdoa.py::test_scan_coincidences_can_disagree_with_associate_on_the_seed
    for the reproduction; fixing the mismatch itself (mirror the release rule here, or stop
    matching by seed) is a bigger, separate change and not this docstring's job.
