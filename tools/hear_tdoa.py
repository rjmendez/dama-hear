...
WHAT IT WRITES, all under `--out` (default `<pool>/tdoa`) and NEVER inside the checkout:
    ...
    <out>/runs/<run_id>/manifest.json     policy, survey, window, funnel, conservation,
                                          not_composed, could_not_do, timings
...
a content address of the member SET. Consequences, published rather than hidden: a backfill that
adds a member produces a NEW event_key and the superseded row stays (counted
`event_membership_changed`); a backfill older than the lookback can never be associated (counted
...
DROP_REASONS = (D_UNANCHORED, D_OUTSIDE_WINDOW, D_OUTSIDE_LOOKBACK_EMITTED,
                ...
                D_CLOCK_UNSTATED, D_CLOCK_UNTRUSTED, D_SYNC_SIGMA, D_ONSET, D_LATENCY,
                D_UNPARSEABLE)
...
def event_key(member_pool_keys: Sequence[str], source_class: Optional[str] = None) -> str:
    """A content address of the MEMBER SET and analysis profile.
    ...
    The same arrivals can be evaluated under different required source classes. Those are
    different claims and must not suppress each other merely because their member set matches.
    `None` preserves the public helper's previous member-only behavior for callers that do not
    have an analysis profile.
    """
    h = hashlib.sha256()
    if source_class is not None:
        h.update(("source_class=" + str(source_class) + "\x1e").encode())
...
def read_event_keys(out: str) -> set:
    ks = set()
    for _day, path in _partitions(arrivals_dir(out)):
        with open(path) as fh:
            for line in fh:
                ...
                if not line:
                    continue
                try:
                    k = json.loads(line).get("event_key")
                except Exception:
                    continue
                if k:
                    ks.add(k)
...
def run(pool_root: str, survey_path: str, policy: Dict[str, Any], out: Optional[str] = None,
    ...
    prior_event_keys = read_event_keys(out)
    ...
    for gi, group in enumerate(candidates):
        ...
        ek = event_key(pkeys, policy["source_class"])
    ...
    if write:
        ...
        report["event_membership_changed"] = sum(
            1 for e in events_out if e["event_key"] not in prior_event_keys and prior_event_keys)
...
def main(argv=None) -> int:
    ...
    policy: Dict[str, Any] = {
        ...
        "clock_unstated": a.clock_unstated, "latency_cal": cal,
