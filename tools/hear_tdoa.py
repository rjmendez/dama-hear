...
DROP_REASONS = (D_UNANCHORED, D_OUTSIDE_WINDOW, D_OUTSIDE_LOOKBACK_EMITTED,
                ...
                D_ONSET_UNSTATED, D_LATENCY, D_HETEROGENEOUS_CLASS, D_UNPARSEABLE)

# ---------------------------------------------------------------- heterogeneous receivers
# WEIGHTING HANDLES VARIANCE. IT DOES NOT HANDLE BIAS. `hear/solve/point.py` has no sigma or
# weight argument -- every admitted arrival votes at equal weight -- so there is nowhere to put
# a declared clock sigma even once nodeclass.py accepts one. A receiver's capture-path BIAS is
# worse than unweighted: it is a constant per-receiver offset that does not average down over
# events and is invisible in an exactly-determined fit (nodeclass.py's module docstring). So a
# second, independent question sits in front of the CLOCK gates below: does this receiver share
# the array's own capture path at all, so that an identical, uncancelled bias is at least the
# SAME uncancelled bias on every member. This is that door, and it is closed by default.
#
# ⚠️THIS IS NOT A SECOND COPY OF nodeclass.contributes_arrival(). That question is "is this
# receiver's clock and bias small enough, by itself". This one is "does this receiver's capture
# path cancel against the ones already in the array" -- nodeclass.py says outright that it does
# NOT model the answer ("⚠️WHAT THIS MODEL DOES NOT DO"): path_bias_s is carried as if it never
# cancels, which is conservative for a SAME-class pair (mach/rankine truly do share one PDM path
# and one firmware) and silent about a DIFFERENT-class pair, where it is the live risk. Both
# doors have to open before a mixed-class arrival is admitted.
