...
def _work_order_meta(root: Optional[str], row: Dict[str, Any]) -> Dict[str, Any]:
    p = row.get("path")
    basename = (os.path.basename(p) if isinstance(p, str) else "") or os.path.basename(
        row.get("clip") or "")
    mtime = None
    if root and isinstance(p, str):
        ...
        if os.path.exists(ap):
            try:
                ...
                mtime = None
    return {"day": _row_day(row),
            "mtime": float(mtime if mtime is not None else (row.get("fetched_at")
                                                             or row.get("ts_utc_s") or 0.0)),
            "node": row.get("node") or "",
            "basename": basename}


def work_order(index: Dict[str, Dict[str, Any]], root: Optional[str] = None) -> List[Dict[str, Any]]:
    """Every indexed clip, IN THE SAME ORDER `clips.prune()` WOULD EVICT ITS AUDIO.

    ⚠️ORDER IS BY PRUNE RISK. `clips.prune()` deletes `unanchored` clips first by measured file
    mtime, then the dated days oldest-first. Tagging against any other order leaves the clips most
    likely to disappear at the back of the queue.
    """
    return sorted(index.values(),
                  key=lambda r: (CLIPS._prune_order(_work_order_meta(root, r)),
                                 r.get("clip_key") or ""))
...
def run(root: str, *, model_dir: str, limit: int = DEFAULT_LIMIT,
    ...
    t0 = time.time()
    stop_reason = None
    for row in work_order(index, root):
