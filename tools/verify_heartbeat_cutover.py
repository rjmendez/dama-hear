...
#: The sixth, and it is a consequence of dropping the `code` volume rather than a separate
#: liberty. deploy/k8s/gen_configmap.py syncs `checksum/<bundle>` into the applied Deployment's
#: pod template so that regenerating the ConfigMap forces a restart of the pod that subPath-mounts
#: it. The proposed pod mounts no ConfigMap -- the code is in the image, and the image digest is
#: what changes the template now -- so the annotation has nothing left to force and goes with the
#: mount. tests/test_service_images.py::test_the_proposed_manifest_changes_only_packaging
#: normalises it away for the same reason; this is that rule, in the instrument.
ALLOWED_DROPPED_POD_ANNOTATIONS = ("checksum/%s" % BUNDLE,)
...
def _pod_meta(deployment):
    try:
        return deployment["spec"]["template"].setdefault("metadata", {})
    except (KeyError, AttributeError, TypeError):
        raise InputError("deployment has no pod template metadata")
...
def diff_manifests(applied_path, proposed_path):
    ...
    for old, new in zip(old_docs, new_docs):
        ...
        # The ConfigMap-restart annotations, dropped only when the mount they force a restart
        # for is dropped too. A proposed manifest that keeps one (with any value) is compared
        # on it exactly as before: this removes the key from the applied side only when the
        # proposed side does not carry it at all.
        old_ann = _pod_meta(old_norm).get("annotations") or {}
        new_ann = _pod_meta(new_norm).get("annotations") or {}
        for key in ALLOWED_DROPPED_POD_ANNOTATIONS:
            if key in old_ann and key not in new_ann:
                old_ann.pop(key)
        if not old_ann:
            _pod_meta(old_norm).pop("annotations", None)
