...
| 5 | **`gold`** | `esp32s3-i2s-gps` **`-qspi`** quad | **NO-GO for this rollout.** `gold` has 2 MiB PSRAM against the old 2.88 MiB minimum ring, so it currently reports `psram=0` with no raw ring and no PCM proof. Building the quad asset (§2) is required. The ring-sizing decision is now made in firmware — `PRAW_TIERS_S` continues below 30 s to 20/15/10 (`firmware/hear_node/README.md`, "the small-PSRAM tiers"), reachable only where every larger tier already failed — so an image carrying it gives `gold` a ring once the quad build is installed. Installing it is still owned by `gold-quad-flash-readiness` and is **not** part of this rollout: ship the asset, do not flash the node. |
