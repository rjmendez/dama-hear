...
register(NodeClass(
    ...
    #
    # THE HARDWARE. hugbot5000's Raspberry Pi 5 carries three ESP32-S3 audio boards on USB as UAC
    # devices at 48 kHz: two Waveshare ESP32-S3-AUDIO-Boards (left and right, GNSS PPS wired to
    # GPIO 4) and one Espressif ESP32-S3-BOX-3 (rear, PPS not wired). Each board's ES7210 has two
    # of its four channels wired to microphones 38.1 mm apart, so the array is six microphones in
    # three pairs (hugbot5000 firmware/esp32s3-audioboard-uac/main/board_pins.h). The 16x ICS-52000
    # TDM array is a separate build document in hugbot5000 (docs/MIC_ARRAY_3D.md), not this array.
    #
    # WHY IT IS REFUSED. Two measurements, both on the USB capture path, not on the clock:
    ...
    #     the other two boards aligned in 6/150 and 7/150 and were refused. A lag that spreads by
    #     ~200 ms is not a constant that subtracts, and at the time the ring's t_anchor_ns was 0, so
    #     no ring frame could be dated at all.
    # Either alone is orders of magnitude outside the arrival bias budget (91.5 us, 31.4 mm).
    #
    # The boards themselves latch their ADC sample counter on each PPS edge and measure their rate
    # against GPS (hugbot5000 firmware pps_clock), so a timing path that dates samples on the board
    # rather than at USB delivery is not ruled out. Until one is measured, this class stays refused.
    #
    # ⚠️BUT HUGBOT IS A USEFUL SENSOR NODE. Three co-located microphone pairs give detection,
    # classification and per-pair bearings (capped at 4501 Hz by the 38.1 mm spacing), and hugbot
    # publishes the fleet's 172 B sketch (hugbot5000 perception/hear_sketch.py). It is excluded from
    # arrivals; it is not excluded from the pool.
    t_sigma_s=3e-3,  # system time disciplined by chrony; no per-sample capture timestamp is measured
    # The entire budget is clock and no capture term has been measured to split it.
    clock_sigma_s=3e-3,
    # THE DISQUALIFYING TERM: the measured USB capture latency above. Even if its 11.3 ms spread were
    # treated as a capture sigma, the 178.5 ms itself is a bias that does not average, and the
    # ring-to-UTC lag varies too much across trials and boards to be subtracted.
    ...
    power_profile="battery_mobile",
    notes="hugbot5000: Raspberry Pi 5 with two Waveshare ESP32-S3-AUDIO-Boards and one "
          "ESP32-S3-BOX-3 over USB, six microphones in three 38.1 mm pairs at 48 kHz. Useful for "
          "detection, classification and per-pair bearings. Refused for arrivals: the USB capture "
          "path carries a measured 178.5 ms latency (61.2 m of bias, self-loopback bench "
          "2026-09-04) and a ring-to-UTC lag spread of p05 34.6 to p95 244.7 ms across 150 trials "
          "(2026-09-10).",
))
...
# tools/calibrate_claps.py and tools/hear_latency_cal.py both measure a class's capture-path
# delay against an external reference and write it out as config/calibrated_node_biases.json
# (schema "hear.calibrated_node_biases.v1"). Loading that file is how a class moves from
# `path_bias_s=None` -- NEVER MEASURED, a refusal -- to a stated bias that require_arrival() can
# admit. The confidence gate below is deliberately SEPARATE from ARRIVAL_PATH_BIAS_MAX_S: a
# calibration run can converge on a small bias with a large uncertainty, and a wide sigma_b_s is
# a reason not to trust the number yet, independent of whether the number itself would pass.
CALIBRATION_SIGMA_MAX_S = 30e-6   # 30 us: the confidence bound this module requires of a
                                  # solved bias before it is trusted enough to apply automatically.

_DEFAULT_CALIBRATION_PATH = "config/calibrated_node_biases.json"


def apply_calibration(node_class_name: str, path_bias_s: float, sigma_b_s: float,
                      *, sigma_max_s: float = CALIBRATION_SIGMA_MAX_S) -> NodeClass:
    """Apply ONE measured capture-path bias to an already-registered class.

    This is the single place `path_bias_s` is allowed to change after registration -- the
    constructor validates it once, and this function re-validates the same way rather than
    poking the attribute directly, so a bad calibration run cannot silently corrupt a class.

    `sigma_b_s` is the calibration solve's own uncertainty on the bias it produced (e.g.
    `ClapCalibrationResult.sigma_b_s`), NOT `t_sigma_s` and NOT `ARRIVAL_PATH_BIAS_MAX_S` -- it
    answers "how sure is this measurement", not "is the measured value small enough". A bias is
    only applied when BOTH the measurement is trusted (`sigma_b_s <= sigma_max_s`) and the
    resulting class subsequently passes `capture_bias_bounded()` on its own terms; this function
    does the former and leaves the latter to `contributes_arrival()` / `require_arrival()`, so a
    class can be measured-but-still-too-biased without this function lying about it.

    Raises CapabilityError, and leaves the class untouched, if the measurement is not trustworthy
    or not a legal `path_bias_s` (see NodeClass.__init__ for what "legal" means -- finite,
    positive, non-zero).
    """
    cls = get(node_class_name)
    if sigma_b_s is None or not math.isfinite(sigma_b_s) or sigma_b_s < 0.0:
        raise CapabilityError(
            "%s: sigma_b_s must be a finite non-negative number of seconds, got %r"
            % (node_class_name, sigma_b_s))
    if sigma_b_s > sigma_max_s:
        raise CapabilityError(
            "%s: calibration sigma_b %.1f us exceeds the %.1f us confidence gate this module "
            "requires before trusting a solved bias -- refusing to apply an uncertain "
            "measurement rather than admit a class on a number that might move"
            % (node_class_name, sigma_b_s * 1e6, sigma_max_s * 1e6))
    if path_bias_s is None or not math.isfinite(path_bias_s):
        raise CapabilityError(
            "%s: path_bias_s must be a finite number of seconds, got %r"
            % (node_class_name, path_bias_s))
    # path_bias_s is stored as a MAGNITUDE (see NodeClass.__init__ / the module docstring); a
    # calibration solve reports a signed offset (direction matters for correcting a timestamp),
    # so the sign is dropped on the way in, exactly as every hand-written registration above does.
    magnitude = abs(float(path_bias_s))
    if magnitude == 0.0:
        raise CapabilityError(
            "%s: calibration solved path_bias_s == 0.0, which no capture hardware supports -- "
            "treat this as a measurement error, not a perfect capture path" % (node_class_name,))
    cls.path_bias_s = magnitude
    return cls


def load_calibrated_biases(path: Optional[str] = None,
                          *, sigma_max_s: float = CALIBRATION_SIGMA_MAX_S) -> List[str]:
    """Read a `hear.calibrated_node_biases.v1` file and apply every trustworthy bias in it.

    `path` defaults to `config/calibrated_node_biases.json` at the repo root (the default output
    of tools/calibrate_claps.py). Missing file -> no-op, returning `[]`, so importing this module
    before a calibration run has ever produced anything is not an error.

    A node entry is applied only if ALL of the following hold, and is silently skipped (not
    raised) otherwise -- a marginal or partial calibration run should leave a class exactly as
    refused as it already was, not half-apply it:
      * its `node_id` names a class already registered in CLASSES;
      * its `status` is "admissible" (calibrate_claps.py's own tolerance gate), if present;
      * it states both `path_bias_s` and `sigma_b_s`;
      * `sigma_b_s <= sigma_max_s` and `path_bias_s` is a legal, non-zero, finite number.

    Returns the sorted list of class names actually updated.
    """
    if path is None:
        path = str(Path(__file__).resolve().parents[1] / _DEFAULT_CALIBRATION_PATH)
    p = Path(path)
    if not p.exists():
        return []
    ...
    for node_id, entry in nodes.items():
        if node_id not in CLASSES or not isinstance(entry, dict):
            continue
        status = entry.get("status")
        if status is not None and status != "admissible":
            continue
        path_bias_s = entry.get("path_bias_s")
        sigma_b_s = entry.get("sigma_b_s")
        if path_bias_s is None or sigma_b_s is None:
            continue
        try:
            apply_calibration(node_id, path_bias_s, sigma_b_s, sigma_max_s=sigma_max_s)
