...
def _geom_str(g: Tuple[Any, Any, Any, Any]) -> str:
    ...
    fmt = lambda v: "?" if v is None else ("%g" % float(v))       # noqa: E731
    ...
    def scene_matrix(self, node: Optional[str] = None, day: Optional[str] = None,
        ...
        for r in self.scene(node=node, day=day):
            ...
            if geom is None:
                ...
                if None in (geom[2], geom[3], g[2], g[3]):
                    why = ("one of them leaves its band axis UNDECLARED (an S0 row carries no "
                    ...
                    why = ("the shape can match while the band EDGES do not, and then band k is "
                ...
                raise ValueError("mixed scene geometry: %s then %s -- %s. Select a day or node "
                                 ...
                                 % (_geom_str(geom), _geom_str(g), why))
